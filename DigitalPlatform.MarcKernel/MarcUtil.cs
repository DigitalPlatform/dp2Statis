...
namespace DigitalPlatform.Marc
{
    ...
    out string strError)
            ...
            out string strError)
        {
            ...
            var nodes = dom.DocumentElement?.SelectNodes("//unimarc:leader | //unimarc:controlfield | //unimarc:datafield | //usmarc:leader | //usmarc:controlfield | //usmarc:datafield", nsmgr); // | //dprms:file
            if (nodes != null)
            {
    ...
    out string strError)
        {
            ...
            foreach (XmlElement field in controlfields)
            {
                // XmlElement field = controlfields[i];
                string strTag = field.GetAttribute("tag");
                if (strTag.Length != 3)
                {
                ...
                string strContent = field.InnerText;    // DomUtil.GetNodeText(field);
            ...
            foreach (XmlElement field in datafields)
            {
                ...
                string strInd1 = field.GetAttribute("ind1");
                if (strInd1.Length != 1)
                {
                ...
                string strInd2 = field.GetAttribute("ind2");
                if (strInd2.Length != 1)
                {
                ...
                foreach (XmlElement subfield in subfields)
                {
                    ...
                    {
                        // 前导纯文本
                        strContent.Append(/*DomUtil.GetNodeText(subfield)*/subfield.InnerText);
                    ...
                    string strSubfieldContent = subfield.InnerText; // DomUtil.GetNodeText(subfield);
            ...
            out string strError)
        {
            ...
            // '//'保证了无论MARC的根在何处，都可以正常取出。
            var root = dom.DocumentElement.SelectSingleNode("//marcxchange:record", nsmgr) as XmlElement;
            if (root == null)
            {
            ...
            string strFormat = root.GetAttribute("format");
            if (strFormat != null)
            ...
            foreach (XmlNode field in root.ChildNodes)
            {
                ...
                if (field.LocalName == "controlfield" && field.NamespaceURI == strMarcXUri)
                {
                    ...
                    foreach (XmlAttribute attr in field.Attributes)
                    {
                        ...
                        target_field.SetAttribute(attr.Name, attr.Value);
                    }
                    continue;
                }
                ...
                if (field.LocalName == "datafield" && field.NamespaceURI == strMarcXUri)
                {
                    ...
                    foreach (XmlNode child in field.ChildNodes)
                    {
                        ...
                        {
                            XmlElement target_subfield = target_dom.CreateElement(strPrefix + ":subfield", strMarcNs);
                            ...
                            foreach (XmlAttribute attr in child.Attributes)
                            {
                                ...
                                target_subfield.SetAttribute(attr.Name, attr.Value);
                            }
                        }
                    }
                    continue;
            ...
            out string strError)
        {
            ...
            target_dom.DocumentElement.SetAttribute(
                "format",
                strFormat);
            target_dom.DocumentElement.SetAttribute(
    "type",
            ...
            foreach (XmlNode source_field in root.ChildNodes)
            {
                ...
                if (source_field.LocalName == "datafield" && source_field.NamespaceURI == strMarcNs)
                {
                    ...
                    foreach (XmlNode child in source_field.ChildNodes)
                    {
                        ...
                        {
                            XmlElement target_subfield = target_dom.CreateElement("marc:subfield", strMarcXUri);
                            target_field.AppendChild(target_subfield);
                            target_subfield.InnerText = child.InnerText;
                            foreach (XmlAttribute attr in child.Attributes)
                            {
